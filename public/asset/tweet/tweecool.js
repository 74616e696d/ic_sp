	...
	$.fn.extend({
		...
		tweecool : function(options) {
			...
			return this.each(function() {
				var o = options;
				var wrapper = $(this);
				var wInner = $('<ul class="media-list">').appendTo(wrapper);
					...
					$.each(data.tweets, function(i, field) {
                                                ...
                                                if (o.profile_image) {
                                                    if( o.profile_img_url == 'tweet' ){
                                                        pIMG = '<div class="media-left"><a href="https://twitter.com/' + o.username + '/status/'+field.id_str+'" target="_blank"><img src="' + data.user.profile_image_url + '" alt="' + o.username + '" /></a></div>';
                                                    }else{
                                                        //pIMG = '<div class="media-left"><a href="https://twitter.com/' + o.username + '" target="_blank"><img class="media-object" src="' + data.user.profile_image_url + '" alt="' + o.username + '" /></a></div>';
                                                        pIMG = '<div class="media-left"><a href="https://twitter.com/' + o.username + '" target="_blank"><i style="font-size:24px;" class="fa fa-twitter"></i></a></div>';
                                                    }
                                                }else{
                                                    pIMG = ''; 
                                                }
						...
						if (o.show_time) {
						    timestamp = xTimeAgo(field.timestamp);
                                                ...
                                                if( o.show_actions ){
                                                   abox = '<div class="action-box"><ul class="media-list">';
						...
						wInner.append('<li class="media">' + pIMG + '<div class="tweets_txt media-body">' + mtext.replace(urlpattern, '<a href="$1" target="_blank">$1</a>').replace(usernamepattern, '<a href="https://twitter.com/$1" target="_blank">@$1</a>').replace(hashpattern, '<a href="https://twitter.com/search?q=%23$1" target="_blank">#$1</a>') + media + ' <span>' + timestamp + '</span>'+abox+'</div></li>');
					});
