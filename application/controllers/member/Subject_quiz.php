...
use Carbon\Carbon;
class Subject_quiz extends Member_controller {
	...
	function __construct()
	{
		...
		$this->load->model('subject_quiz_model','sqm');
		$this->load->model('subject_quiz_summery_model','sqsm');
	}
	public function index()
	{
		$sub_id=$this->uri->segment(4);
		$questions=$this->question_bank_model->get_subject_question_rand($sub_id);
       ...
       if($questions)
		...
		$data['subject_name']=$sub_id;
		$data['user']=$this->userid;
		$data['last_ques']=$last;
		$data['questions']=$str_ques;
		$data['title']='';
		$this->load->blade('member.subject_quiz', $data);
	}
		...
		function finish_exam()
		{
			...
			if(count($dt)>2)
			{
				...
				foreach ($dt as $d) 
				{
					if($i==1)
					{
						$subject_name=$d->test_name;
						$time_taken=$d->timeTaken;
						$quiz_id=$this->generate_test_id($subject_name,$user);
					}
					else
					{
						$correct_ans=$this->get_correct_answer($d->sub_qid);
						if($d->sub_sl==$correct_ans)
						{
							$correct_count++;
						}
						else
				...
				$this->sqsm->create($data_summery);
				echo base_url()."member/subject_quiz_result/index/{$quiz_id}/$user";
...
/* End of file subject_quiz.php */
/* Location: ./application/controllers/member/subject_quiz.php */