...
class Edit_question extends CI_Controller {
  ...
  var $msg='';
	...
	public function index()
	{
	    ...
	    $data['ques']=$ques;
	    $data['edit_id']=$id;
	    $data['cats']=$this->ref_text_model->get_ref_text_by_group(1);

	    $data['exam_name_val']=explode(',',$ques?$ques->exam_name:'');
	    $data['sbj_val']=$ques?$ques->subject:false;
	    $data['chapter_group_val']=$ques?$ques->chapter_group:'';
	    $data['chapter_val']=$ques?$ques->chapter:false;

      $cpt_grp=$ques?$ques->chapter_group:false;

      $chapters=$this->ref_text_model->where(array('group_id|='=>4,'parent_id|='=>$cpt_grp))->get();

	    $data['subjects']=$this->ref_text_model->get_ref_text_by_group(2);
	    $data['chapter_group']=$this->ref_text_model->get_ref_text_by_group(3);
      ...
      $data['sel_prev_exam']=$this->exam_model->prev_test_by_ques($id);
  		...
  		$this->load->blade('admin.question.edit_question',$data);
	}
        ...
        $rules=array('ddl_subject|Subjects'=>'min:0',
         ...
         if($this->my_validation->validate($_POST,$rules))
         {
             ...
             $this->session->set_flashdata('success','successfully updated!');
              redirect(base_url()."school/admin/questions/index");
              }
              else
              {
                $msg=$this->my_validation->errors;
                $this->session->set_flashdata('msg',$msg);
                redirect(base_url()."school/admin/questions/index/{$edit_id}");
              }
