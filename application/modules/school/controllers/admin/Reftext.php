...
class Reftext extends Schooladmin_Controller {
		...
		function ref_list_dt()
	    {
	      ...
	      if($list)
	      {
	          foreach($list as $item)
	          {
	              ...
	              $row[] = $item->serial;
	              $add_lnk=base_url().'school/admin/reftext/child_ref_text/'.$item->id;
	              $edit_lnk=base_url().'school/admin/reftext/edit/'.$item->id;
	              $action='';
	...
	function ref_group_table()
	{
		...
		if($groups)
		{
			foreach($groups as $rg)
			{
				...
				$str.="<td>{$rg->name}</td>";
				$str.="<td>";
				$new_ref_text_lnk=base_url().'school/admin/reftext/new_ref_text/grp/'.$rg->id;
	...
	function save_child_ref_text()
	{
		...
		echo success_message("Reference Text saved successfully");
	}
	...
	function update()
	{
		...
		echo success_message("Reference text update successfully !");
	}
	...
	function add_ref_rext()
	{
		...
		echo success_message('Reference text added successfully !');
	}
	...
	function delete()
	{
		...
		redirect(base_url().'school/admin/reftext');
	}
}
