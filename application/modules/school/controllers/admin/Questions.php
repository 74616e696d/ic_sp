...
class Questions extends Schooladmin_Controller {
	...
	function question_list_dt()
	{
	    ...
	    if($list)
	    {
	        foreach($list as $q) 
	        {
	            ...
	            $row[]=$q->hints;
	            $action="<a href='".base_url()."school/admin/edit_question/index/{$q->id}' class='ttp' data-toggle='tooltip' data-placement='top' title='Edit'><i class='icon-edit'></i></a>";
	            $action.="<a onclick='return(confirm(\"are you sure to delete?\"))' href='".base_url()."school/admin/question_bank/deletequestion/{$q->id}'class='ttp' data-toggle='tooltip' data-placement='top' title='Delete'><i class='icon-trash'></i></a>";
	            $row[] = $action;
	            $data[] = $row;
	        }
	    }
	...
	function create()
	{
		$data['cats']=$this->reftext_model->get_ref_text_by_group(1);
		$data['subjects']=$this->reftext_model->get_ref_text_by_group(2);
		$data['chapter_group']=$this->reftext_model->get_ref_text_by_group(3);
	...
	function store()
	{
        ...
        if($this->my_validation->validate($_POST,$rules))
        {
            ...
            if($save)
            {
                ...
                redirect(base_url().'school/admin/question_bank');
            }
            if($save_new)
            {
                $this->session->set_flashdata('success','Question added successfully!');
                ...
                redirect(base_url().'school/admin/add_question');
            }
        }
        else
        {
	...
	function get_subjects()
	{
	    ...
	    $exam_names=$this->reftext_model->get_ref_text_by_parent_group($cid,2);
	    $str='';
