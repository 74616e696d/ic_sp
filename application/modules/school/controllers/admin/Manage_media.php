...
class Manage_media extends Schooladmin_Controller {
	...
	function get_media_files($term='')
	{
		...
		$files=$finder->files()->in('asset/school/media/');
		$ignored=['.','.htaccess','.git','.quarantine','.tmb'];
		//$files=$this->sort_file_file_last_modi('asset/school/media/');
		$str='';
		if(count($files)>0)
		{
			...
			foreach ($files as $file) 
			{
			   ...
			   if(file_exists('asset/school/media/'.$fl)){
			   //$str.="<li class='span3'>";
			   $copy_link=base_url().'asset/school/media/'.$fl;
			   $str.="<article class='ripplelink' data-clipboard-text='{$copy_link}' data-img='{$copy_link}'>";
			   $src=base_url().'asset/school/media/'.$fl;
	...
	function send()
	{
		...
		else
		{
			...
			if(in_array($ext, $allowed))
			{
				...
				$img->save("asset/school/media/{$img_name}");
				echo "ok";
			}
			else
			{
	...
	function remove()
	{
		...
		if(file_exists("./asset/school/media/".$name))
		{
			unlink("./asset/school/media/".$name);
