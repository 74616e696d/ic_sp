...
@section('content')
    <input type="hidden" id='hdn_test_name' value="{{$chapter_name;}}">
...
<div class="col-sm-12 col-xs-12 visible-xs visible-sm toolbar-mobile">
  ...
  <div class="pull-left"><h4>{{$test_meta->name}}</h4>
   <!-- <div class='clockDiv clock-mobile'></div>  -->
  </div>
  <div id='exp-calc' class='pull-right'><i class="fa fa-calculator"></i></div>
</div>
<div class='col-sm-12 col-xs-12 col-md-12 clockDiv clock-mobile visible-xs visible-sm'></div> 
    <section class="row top-buffer">
      <div id='left-portion' class="col-lg-4 col-md-4 col-sm-10 col-xs-10">
        <div id="sidebar" role="navigation">
          <div class="well sidebar-nav">
            <h2 class="hidden-sm hidden-xs title tutorial-color">{{$test_meta->name}}</h2>
  ...
  <?php
    $time=!empty($test_meta->time)?$test_meta->time:20;
  ?>
@stop
...
<script type='text/javascript'>
  var store_ans_url='{{$base_url}}member/chapter_quiz/store_ans',
  finish_action='{{$base_url}}member/model_quiz/finish_exam',
  report_action='{{$base_url}}member/chapter_quiz/report_question',
  review_action='{{$base_url}}member/test/add_to_review',
  get_mrk_ques_action='{{$base_url}}member/chapter_quiz/marked_questions',
  get_all_ques_action='{{$base_url}}member/chapter_quiz/all_questions';
...
<script type="text/javascript" src='{{$base_url}}asset/js/model_quiz.js'></script>
<script type="text/javascript">
...
// show confirmation  before page leave or refresh
...
function showAns(qid, sl, test_name) {
  ...
  $.ajax({
      url: '{{$base_url}}member/chapter_quiz/get_ans',
      type: 'GET',
      data: {
        qid: qid,
        sl: sl,
