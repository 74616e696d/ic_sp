			...
			<!--<div class="tab-content">
			  <div role="tabpanel" class="tab-pane active" id="bcs">
			  	<table class="table table bordered">
			  		...
			  		<tbody>	
			  			@if($bcs_subject)
			  			@foreach($bcs_subject as $sbj)
			  			<tr>
			  				<td>{{$sbj->name}}</td>
			  				<td><?php echo $this->refTextModel->total_subj_chapter($sbj->id) ?></td>
			  				<?php $chapters=Ref_text_model::get_subjects_chapter($sbj->id); 
			  				$attempted_bcs_chapter=quiz_summery_model::attempted_chapter_quiz($user,$chapters);
			  				?>
			  				<td>{{$attempted_bcs_chapter}}</td>
			  				<?php
			  				$bcs_correct=quiz_summery_model::attempted_chapter_correct($user,$chapters);
			  				$bcs_marks=$attempted_bcs_chapter*20;
			  				$bcs_score=$bcs_marks!=0?($bcs_correct/$bcs_marks)*100:0;
			  				$bcs_score=$attempted_bcs_chapter==0?'--':number_format($bcs_score,2).'%';
			  				 ?>
			  				<td>{{$bcs_score}}</td>
			  			</tr>
			  			@endforeach
			  ...
			  <div role="tabpanel" class="tab-pane" id="bank">
			  	<table class="table table bordered">
			  		...
			  		<tbody>	
			  			@if($bank_subject)
			  			@foreach($bank_subject as $cpt)
			  			<tr>
			  				<td>{{$cpt->name}}</td>
			  				<td>{{ref_text_model::total_subj_chapter($cpt->id)}}</td>
			  				<?php 
			  					$chapters=ref_text_model::get_subjects_chapter($cpt->id);
			  					$attempted_bank_chapter=quiz_summery_model::attempted_chapter_quiz($user,$chapters);
			  				 ?>
			  				<td>{{$attempted_bank_chapter}}</td>
			  				<?php
			  				$bank_correct=quiz_summery_model::attempted_chapter_correct($user,$chapters);
			  				$bank_marks=$attempted_bank_chapter*20;
			  				$bank_score=$bcs_marks!=0?($bank_correct/$bank_marks)*100:0;
			  				$bank_score=$attempted_bank_chapter==0?'--':number_format($bank_score,2).'%';
			  				?>
			  				<td>{{$bank_score}}</td>
			  			</tr>
			  			@endforeach
