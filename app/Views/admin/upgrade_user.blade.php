...
<div id="pass_dlg" class="modal hide fade">
	<form method="POST" action="<?php echo base_url(); ?>admin/upgrade_user/change">
...
<div id="edit_mem_dlg" class="modal hide fade">
	<form method="POST" action="<?php echo base_url(); ?>admin/upgrade_user/update_membership">
	<div class="modal-header">
   	 <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var role=$('#ddl_role'),
			...
			skey3=-1,
			skey4=0;
			make_url='<?php echo base_url(); ?>admin/upgrade_user/index/'+skey1+'/'+skey2+'/'+skey3+'/'+skey4;
		...
		role.change(function(){
			...
			if(skey1!=-1){
				...
				make_url='<?php echo base_url(); ?>admin/upgrade_user/index/'+-1+'/'+skey2+'/'+skey3+'/'+skey4;
			}
		...
		st_active.click(function(){
			...
			if(skey2.length>0){
				...
				make_url='<?php echo base_url(); ?>admin/upgrade_user/index/'+skey1+'/'+-1+'/'+skey3+'/'+skey4;
			}
		...
		st_online.click(function(){
			...
			if(skey3.length>0){
				...
				make_url='<?php echo base_url(); ?>admin/upgrade_user/index/'+skey1+'/'+skey2+'/'+-1+'/'+skey4;
			}
		...
		$('.activate').click(function(){
			...
			$.ajax({
				url: '<?php echo base_url(); ?>admin/upgrade_user/activate_user',
				type: 'GET',
		...
		$('#edit_mem_dlg').on('hidden', function () {
  		$(this).removeData('modal');
		});
		    ...
		    $('#username').autocomplete({
		        ...
		        change:function( event, ui ) 
		        {
		        	...
		        	if(skey4>0)
		        	 ...
		        	 {
		        	     make_url='<?php echo base_url(); ?>admin/upgrade_user/index/'+skey1+'/'+skey2+'/'+skey3+'/'+0;
