...
@section('content')
<form method="post" action="{{$base_url}}admin/written_ques/update" class="form-horizontal">
<input type="hidden" name="hdn_id" value="{{$ques->id}}">
		...
		<div class="control-group">
			...
			<select required='required' name="category" id="category">
				...
				@if($cats)
				@foreach($cats as $ct)
				<?php $sel_cat=$ques->category==$ct->id?'selected':''; ?>
		...
		<div class="control-group">
			...
			<div class="controls">
				<textarea name="ques" required='required' id="ques" cols="30" rows="10">{{$ques->question}}</textarea>
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				<textarea name="ans" id="ans" cols="30" rows="10">{{$ans->answer}}</textarea>
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				<button type="submit" name='save' class='btn btn-info' value="1"><i class="fa fa-save"></i> Update</button>
...
$(document).ready(function() {
	...
	//setting old value
	var old_cat='{{$ques->category}}';
	if(old_cat.length>0){
		var old_exam='{{json_encode($exams)}}';
		$.ajax({
