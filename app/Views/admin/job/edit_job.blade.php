...
<div class="row">
	<form action="{{ $base_url }}admin/job/update" method="post">
	<input type="hidden" name="hdn_id" value="{{ $job->id }}">
	<div class="span7 job-left">
		...
		<select name="ddl_company" id="ddl_company">
			...
			<option {{ $job->com_info==$com->id?'selected':'' }} value="{{ $com->id }}">{{ $com->company_name }}</option>
			@endforeach
			@endif
		</select>
			...
			<select name="job_category" id="job_category">
				...
				@if($categories)
					...
					<option {{ $job->job_cat==$cat->id?'selected':'' }} value="{{ $cat->id }}">{{ $cat->title }}</option>
					@endforeach
				@endif
			</select>

			<label for="txt_title" class="control-label">Job Title</label>
			<input type="text" name="txt_title" id="txt_title" required='required' value="{{ $job->title }}">

			<label for="post_name" class="control-label">Post Name</label>
			<input type="text" name="post_name" id="post_name" value="{{ $job->post_name }}">
				
			<label for="short_desc" class="control-label">Short Description</label>
			<textarea name="short_desc" id="short_desc" style='width:100%'>{{ $job->details }}</textarea>
			
			<label for="txt_job_details" class="control-label">Job Responsibility</label>
			<textarea name="txt_job_details" id="txt_job_details">{{ $job->job_responsibility }}</textarea> 

			<label for="txt_job_requirement" class="control-label">Job Requirements</label>
			<textarea name="txt_job_requirement" id="txt_job_requirement">{{ $job->job_requirements }}</textarea> 

			<label for="txt_apply_instructions" class="control-label">Apply Instructions</label>
			<textarea name="txt_apply_instructions" id="txt_apply_instructions">{{ $job->apply_instructions }}</textarea> 

			<label for="txt_education" class="control-label">Education</label>
			<textarea name="txt_education" id="txt_education" style='width:100%'>{{ $job->education }}</textarea> 

			<label for="txt_exp" class="control-label">Experience</label>
			<input type='text' name="txt_exp" id="txt_exp" value="{{ $job->experience }}">

			<label for="txt_exp_details" class="control-label">Experience Details</label>
			<textarea name="txt_exp_details" id="txt_exp_details" style='width:100%'>{{ $job->experience_requirement_details }}</textarea>

		

			<label for="txt_additional_req" class="control-label">Aadditional Job Requirement</label>
			<textarea name="txt_additional_req" id="txt_additional_req">{{ $job->aditional_job_requirement }}</textarea>
			
	</div>
	<div class="span4 job-right">
		<label for="txt_vacancy" class='control-label'>Number Of Vacancy</label>
		<input type="number" min="0" max="1000" value="{{ $job->vacancy_no }}" name="txt_vacancy" id="txt_vacancy">

		<label for="txt_salary_range" class='control-label'>Salary Range</label>
		<input type="text" name="txt_salary_range" id="txt_salary_range" value="{{ $job->salary_range }}">

		<label for="txt_pub_date" class='control-label'>Publish Date</label>
		<input type="text" name="txt_pub_date" id="txt_pub_date" class="dt" value="{{ $job->publish_date }}">

		<label for="ddl_job_nature" class="control-label">Job Nature</label>
		<select name="ddl_job_nature" id="ddl_job_nature">
			<option {{ $job->job_nature=='Full Time'?'selected':'' }} value="Full Time">Full Time</option>
			<option {{ $job->job_nature=='Part Time'?'selected':'' }} value="Part Time">Part Time</option>
			<option {{ $job->job_nature=='Student'?'selected':'' }} value="Student">Student</option>
			<option {{ $job->job_nature=='Contractual'?'selected':'' }} value="Contractual">Contractual</option>
		</select>
		...
		<input type="text" name="logo" id="logo" value="{{ $job->logo_img }}"> 	
		...
		<input type="text" name="deadline" id="deadline" class="dt" value="{{ $job->deadline }}">

		<label for="location" class="control-label">Job Location</label>
		<select name="location" id="location">
			@if($district)
			@foreach($district as $dis)
			<option {{ $job->location==$dis->FIELD2?'selected':'' }} value="{{ $dis->FIELD2 }}">{{ $dis->FIELD2 }}</option>
			@endforeach
			@endif
		</select>

		<label for="tags" class="control-label">Tags</label>
		<textarea name="tags" id="tags">{{ $job->tags }}</textarea>

		<label for="link" class="control-label">Link</label>
		<input type="text" name="link" id="link" value="{{ $job->link }}">

		<label for="link_text" class="control-label">Link Text</label>
		<input type="text" name="link_text" id="link_text" value="{{ $job->link_text }}">
		<br>
		<label class="checkbox">
			<input type="checkbox" name="ck_display" id="ck_display" {{ $job->is_published?'checked':'' }} value="1">Display
		</label>
		<br>
		<label class="checkbox">
			<input type="checkbox" name="is_featured" id="ck_display" {{ $job->is_featured?'checked':'' }} value="1">Is Featured
		</label>
	</div>
