...
@section('content')
<a class='btn btn-info' href="{{ $base_url }}admin/news">Create News</a>
<br><br>
<div class="form-inline">
	...
	<input type="text" name="post_date" class='dt' id="post_date" placeholder="Post Date">
	<input type="text" name="deadline" class='dt' id="deadline" placeholder="Deadline">
	<select style='width:120px;' name="published" id="published">
		<option value="">All</option>
		<option value="1">Published</option>
		<option value="0">Not Published</option>
	</select>
	<button type="button" id='search' class="btn btn-info"><i class="fa fa-search"></i> Search</button>
</div>
...
jQuery(document).ready(function($) {
	...
	table = $('#job_list').DataTable({
		...
		"ajax": {
		    "url": "{{ $base_url }}admin/news/news_list_dt",
		    "type": "POST"
		},
	...
	$('#search').click(function() {
		...
		var term={cat:cat,pdate:post_date,deadline:deadline,status:status};
...
{
	window.location.href='{{ $base_url }}admin/news/edit/'+id;
}
...
function delete_job(id){
	...
	{
		$.ajax({
			url: '{{ $base_url }}admin/news/remove',
			type: 'GET',
			data: {id: id}
		})
		.done(function() {
