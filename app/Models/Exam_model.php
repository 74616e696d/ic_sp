...
class Exam_model extends Model {
	...
	{
		//$this->db->cache_on();
		$query = $this->db->table('exam')->where('exam_cat',$cat)
		->where('test_type','16');
		if(count($exclude)>0)
		{
			$query = $query->whereNotIn('ref_id',$exclude);
		}
		if(!empty($limit))
		{
			$query = $query->limit($limit);
		}
		$query = $query->select(array('exam.*','ref_text.name'))
		
		->join('ref_text','exam.ref_id=ref_text.id')
		->orderBY('exam.exam_year','DESC')
		->orderBY('ref_text.name','ASC');
	...
	{
		$query = $this->db->table('exam')->where('exam_cat',$cat)->where('test_type','16');
		if(count($exclude)>0){
                    $query = $query->whereNotIn('ref_id',$exclude);
		}
		$query = $query->select(array('exam.*','ref_text.name'))->limit(4)
		
		->join('ref_text','exam.ref_id=ref_text.id');
		if($cat==318)
		{
			$query = $query->orderBY('ref_text.name','ASC');
		}
		else
		{
			$query = $query->orderBY('ref_text.name','DESC');
		}
	...
	{
		return $this->database->table('ref_text')->where('group_id',$group)->where('parent_id',$parent)
		->orderBy('id','desc')
		->limit($limit)
		->get();
		/*if($qry->num_rows()>0)
	...
	{
		return $this->db->table('exam')->select(array('exam.*','ref_text.name'))->join('ref_text','exam.ref_id=ref_text.id')->orderBy('id','desc')->limit($num)->get()->getResultArray();
		/*if($qry->num_rows()>0)
		{
			return $qry->result();
		}
