...
class Slug
{
	...
	//	$CI->load->helper(array('url', 'text', 'string'));
		...
		$string = strtolower($this->slug_url_title($string, $this->replacement));
                $character = $this->_get_replacement();
                
                $str = preg_replace('#' . preg_quote($character, '#') . '{2,}#', $character, $string);

                return (TRUE) ? trim($str, $character) : $str;
	...
	function slug_url_title($str, $separator = '-', $lowercase = FALSE)
	{
		if ($separator == 'dash') 
		{
		    $separator = '-';
